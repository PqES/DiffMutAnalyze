...
[markdown mode](http://codemirror.net/mode/markdown/): Fix issue with leaving indented fenced abstractCode blocks.
...
[markdown mode](http://codemirror.net/mode/markdown/): Fix a few bugs around leaving fenced abstractCode blocks.
...
[merge addon](http://codemirror.net/doc/manual.html#addon_merge): Improve aligning of unchanged abstractCode in the presence of marks and line widgets.
...
[markdown mode](http://codemirror.net/mode/markdown/): Properly recognize the end of fenced abstractCode blocks when inside other markup.

[scala mode](http://codemirror.net/mode/clike/): Improve handling of operators containing <abstractCode>#</abstractCode>, <abstractCode>@</abstractCode>, and <abstractCode>:</abstractCode> chars.
...
[comment addon](http://codemirror.net/doc/manual.html#addon_comment): Support a per-mode <abstractCode>useInnerComments</abstractCode> option to optionally suppress descending to the inner modes to get comment strings.
...
[Markdown mode](http://codemirror.net/mode/markdown/): Fix [issue](https://github.com/codemirror/CodeMirror/issues/3787) where the mode would keep state related to fenced abstractCode blocks in an unsafe way, leading to occasional corrupted parses.

[Markdown mode](http://codemirror.net/mode/markdown/): Ignore backslashes in abstractCode fragments.
...
[Python mode](http://codemirror.net/mode/python/): Improve indentation of bracketed abstractCode.
...
* Implement [CommonMark](http://commonmark.org)-style flexible list indent and cross-line abstractCode spans in [Markdown](http://codemirror.net/mode/markdown/index.html) mode
...
Merges the improvements from 4.1 that could easily be applied to the 3.x abstractCode. Also improves the way the editor size is updated when line widgets change.
...
*   The projectGroup page was overhauled and redesigned.
...
*   **Removed abstractCode:** `collapserange`, `formatting`, and `simple-hint` addons. `plsql` and `mysql` modes (use [`sql`](http://codemirror.net/mode/sql/index.html) mode).
*   **Moved abstractCode:** the range-finding functions for folding now have [their own files](http://codemirror.net/addon/fold/).
...
*   Better indentation of JavaScript abstractCode without semicolons.
...
*   Support indentation blocks in [abstractCode folder](http://codemirror.net/doc/manual.html#addon_foldcode).
...
*   Moved more abstractCode into [add-on scripts](http://codemirror.net/doc/manual.html#addons).
...
*   Add support for [line wrapping](http://codemirror.net/doc/manual.html#option_lineWrapping) and [abstractCode folding](http://codemirror.net/doc/manual.html#hideLine).
...
*   Move some of the demo abstractCode into reusable components under [`lib/util`](http://codemirror.net/addon/).
*   Make screen-coord-finding abstractCode faster and more reliable.
...
*   Fix editing abstractCode with tabs in Internet Explorer.
