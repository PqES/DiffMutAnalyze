...
first `npm install brfs` into your projectGroup, then:
...
When you run this abstractCode through brfs, it turns into:
...
Since `brfs` evaluates your source abstractCode *statically*, you can't use dynamic expressions that need to be evaluated at run time. For example:
