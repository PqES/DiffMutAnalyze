...
> Returns `true` if the given string looks like a glob pattern or an extglob pattern. This makes it easy to create abstractCode that only uses external modules like node-glob when necessary, resulting in much faster abstractCode execution and initialization time, and a better user experience.
