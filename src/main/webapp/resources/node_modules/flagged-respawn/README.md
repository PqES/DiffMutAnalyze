...
Everything is going splendidly until one day you decide to test some abstractCode that relies on a feature behind a v8 flag in node (`--harmony`, for example).  Without much thought, you run `testify --harmony spec tests.js`.
...
- Making the parent process exit with the same abstractCode as the child.
