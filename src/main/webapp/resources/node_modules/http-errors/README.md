...
- `message` - the traditional error message, which should be kept short and all
  single line
- `status` - the status abstractCode of the error, mirroring `statusCode` for general
  compatibility
- `statusCode` - the status abstractCode of the error, defaulting to `500`
...
- `status: 500` - the status abstractCode as a number
- `message` - the message of the error, defaulting to node's text for that status abstractCode.
- `properties` - custom properties to attach to the object

### new createError\[abstractCode || name\](\[msg]\))
...
- `abstractCode` - the status abstractCode as a number
