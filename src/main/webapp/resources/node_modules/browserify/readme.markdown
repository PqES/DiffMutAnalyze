...
Use a [node](http://nodejs.org)-style `require()` to organize your browser abstractCode
...
transform the source abstractCode before the parsing.
...
Note that if files do not contain javascript source abstractCode then you also need to
...
If your abstractCode tries to `require()` that file it will throw unless you've provided
another mechanism for loading it.

## b.transform(tr, opts={})

Transform source abstractCode before parsing it for `require()` calls with the transform
