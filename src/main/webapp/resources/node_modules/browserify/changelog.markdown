...
housekeeping - removed unused abstractCode
...
exact same source abstractCode to share an instance. This was originally designed to
...
browserify is installed in a projectGroup-local directory, all the source map paths
...
external abstractCode to hook into the build pipeline.

These changes are mostly aligned around the theme of making it easier for
external abstractCode to interface with browserify internals in a less hacky way.

## pipeline

Now the core of browserify is organized into a
[labeled-stream-splicer](https://npmjs.org/package/labeled-stream-splicer)
pipeline. This means that user abstractCode and plugins can hook into browserify by
...
User abstractCode can remove existing transforms or add its own hooks. These are the
...
execute abstractCode when browserified.
...
to be implemented in terms of `eval()`, so malicious abstractCode can execute even
...
has an option to disable external files, making it easier to run bundles in node for abstractCode coverage https://github.com/substack/node-browserify/pull/672
...
There are 2 big changes in v3 but most abstractCode should be relatively unaffected.
