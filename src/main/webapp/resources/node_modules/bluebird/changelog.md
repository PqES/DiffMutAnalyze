...
Misc:
 ...
 - Reduce minified full browser build file size by not including unused abstractCode generation functionality.
 - Major internal refactoring related to testing abstractCode and source abstractCode file layout
...
Bugfixes:
 ...
 - `OperationalErrors` thrown by promisified functions retain custom properties, such as `.abstractCode` and `.path`.
...
- [Promisification](API.md#promisification) on steroids - entire modules can now be promisified with one line of abstractCode
...
Bugfixes:
 ...
 - Fix bug with build files containing duplicated abstractCode for promise.js
