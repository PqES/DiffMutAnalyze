...
npm test
```
This runs the `gulp` task `test`. By default the test will be run with the source abstractCode in `lib` directory.

Set the environmental variable `TEST_VERSION` to `compat` to test the transpiled es5-compat version of the abstractCode.

The `gulp` task `test` will always transpile the source abstractCode into es5 and export to `dist` first before running the test.
