...
In order for the UMD wrapper to work the source abstractCode for your module should `return` the export, e.g.
...
options:
 ...
 - `commonJS` (default: `false`) - If commonJS is `true` then it will accept CommonJS source instead of source abstractCode which `return`s the module.
